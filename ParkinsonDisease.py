...
class ParkinsonDisease:
    ...
    def load_datasets(self):
        ...
        # Load test datasets
        self.test_clinical = pd.read_csv("./example_test_files/test.csv")
        self.test_proteins = pd.read_csv("./example_test_files/test_proteins.csv")
        self.test_peptides = pd.read_csv("./example_test_files/test_peptides.csv")
    ...
    def prepare_data(self):
        ...
        # Pivot the proteins and peptides tables so each protein and peptide is a feature
        test_proteins = self.test_proteins.pivot_table(values="NPX", index="patient_id", columns="UniProt")
        test_peptides = self.test_peptides.pivot_table(values="PeptideAbundance", index="patient_id", columns="Peptide")

        # Set the patient id as index
        self.test_clinical = self.test_clinical.set_index(["patient_id"])
